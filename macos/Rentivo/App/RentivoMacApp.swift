...
struct RentivoMacApp: App {
  ...
  private let uiTestWindowSize: CGSize?

  init() {
    #if DEBUG
      let arguments = ProcessInfo.processInfo.arguments
      initialWindowSize = AppLaunchOptions.initialWindowSize(arguments: arguments)
      uiTestWindowSize = AppLaunchOptions.uiTestWindowSize(arguments: arguments)
      let usesMockData =
    ...
    #else
      initialWindowSize = AppLaunchOptions.defaultWindowSize
    #endif
  ...
  var body: some Scene {
    WindowGroup {
      RootView()
        ...
        #if DEBUG
          .background(UITestWindowSizeOverride(size: uiTestWindowSize))
        #endif
    }
