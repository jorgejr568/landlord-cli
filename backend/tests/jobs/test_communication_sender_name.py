...
def _seed_org_comm(conn):
    """Seed an org-owned billing + bill + queued communication; return its id."""
    from rentivo.models.communication import Communication
    from rentivo.repositories.sqlalchemy.communication import SQLAlchemyCommunicationRepository
    from rentivo.services.organization_service import OrganizationService
...
def _capture_send(mod, conn, monkeypatch):
    sent = {}
...
def test_from_uses_communications_from_name(conn, monkeypatch):
    import rentivo.jobs.handlers.communication as mod

    comm_id = _seed_org_comm(conn)
    sent = _capture_send(mod, conn, monkeypatch)
    monkeypatch.setattr(mod.settings, "communications_from_name", "Cobrancas Aurora")
...
def test_from_falls_back_to_ses_from_name(conn, monkeypatch):
    import rentivo.jobs.handlers.communication as mod
