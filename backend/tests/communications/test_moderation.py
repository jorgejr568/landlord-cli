...
def test_normalization_handles_accents_leetspeak_and_repeats():
    for variant in ("mérda", "m3rda", "merdaaaa", "MERDA"):
        assert "merda" in scan(variant).severe, variant
...
def test_legacy_mild_result_is_still_blocked():
    r = ModerationResult(severe=(), mild=("legacy finding",))
    assert r.flagged
    assert r.blocked
