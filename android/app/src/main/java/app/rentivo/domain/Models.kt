...
data class ProfilePIXForm(
  ...
  /**
   * A wholly empty triple explicitly clears/inherits PIX. A partial triple deliberately has no
   * payload, so callers cannot accidentally send data the API would reject or reinterpret.
   */
  val configuration: PixConfiguration?
