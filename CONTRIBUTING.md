...
- CI on PRs (`test-pr.yaml`) runs the `backend`, `frontend`, `e2e`, `migrations`, `compose-config`, `functional-stack`, `production-startup`, and `security-scan` jobs behind `release-gate`. The `production-images` matrix then builds API, worker, and frontend locally with `load: true` so Dockerfile breakage fails the gate, and `all-checks-pass` requires both phases. Image vulnerability scanning is not part of the gate — the weekly `image-vulnerability-scan.yml` workflow owns it.
