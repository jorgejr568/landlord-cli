...
- The browser UI is React/Vite under `frontend/src`; the JSON API is FastAPI under `backend/rentivo/api`. The mobile clients are SwiftUI under `ios/Rentivo` and Jetpack Compose under `android/`.
- Code, comments, and identifiers in **English**; all customer-facing text in **PT-BR** — React, PDF, and email copy, and the iOS and Android app UI alike.
...
- An API schema change must refresh **both** mobile OpenAPI copies: `ios/Rentivo/openapi.json` and `android/app/openapi.json` stay byte-identical to `frontend/openapi.json`. Run `make ios-openapi-sync` and `make android-openapi-sync`, then verify with `make ios-openapi-check` and `make android-openapi-check`.
...
- CI on PRs (`test-pr.yaml`) runs the `changes`, `scripts`, `backend`, `frontend`, `ios`, `android`, `e2e`, `migrations`, `compose-config`, `functional-stack`, `production-startup`, and `security-scan` jobs behind `release-gate`. The `production-images` matrix then builds API, worker, and frontend locally with `load: true` so Dockerfile breakage fails the gate, and `all-checks-pass` requires both phases.
- The `changes` job classifies the diff with three separate classifiers — `scripts/ios-ci.sh paths-changed` gates `ios`, `scripts/android-ci.sh paths-changed` gates `android`, and `scripts/ci-changed-areas.sh` produces the `backend`, `frontend`, `docker`, and `scripts` areas — and the jobs whose inputs are untouched are skipped; skipped jobs count as passing. `scripts` and `security-scan` always run, and any `.github/` change (or an unusable base) marks every `ci-changed-areas.sh` area as changed. Outside pull requests (`workflow_call`, `workflow_dispatch`, push) only those four areas are forced to `true`; `ios` and `android` stay path-filtered on every event.
