...
- CI on PRs (`test-pr.yaml`) runs the `backend`, `frontend`, `e2e`, `migrations`, `compose-config`, `functional-stack`, `production-startup`, and `security-scan` jobs behind `release-gate`. The `production-images` matrix then builds API, worker, and frontend locally with `load: true` so Dockerfile breakage fails the gate, and `all-checks-pass` requires both phases. A `changes` job classifies the diff with `scripts/ci-changed-areas.sh` and skips the jobs whose input areas are untouched — skipped jobs count as passing; `scripts` and `security-scan` always run, and any `.github/` change marks every gate area as changed. Outside pull requests (`workflow_call`, `workflow_dispatch`, push) no path filtering applies and every gate area runs. The `security-scan` job covers backend dependency, secret, and configuration scans; image vulnerability scanning and the frontend npm audit are not part of the gate — the weekly `image-vulnerability-scan.yml` and `npm-audit-report.yml` workflows own them.
