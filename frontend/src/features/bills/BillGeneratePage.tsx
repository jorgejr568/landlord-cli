...
export function BillGeneratePage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const invalidFile = files.map(validateUpload).find((result) => "error" in result);
    if (invalidFile && "error" in invalidFile) {
      setFileError(invalidFile.error);
      document.getElementById("generate_receipt_files")?.focus();
      return;
    }
