  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const items = form.items.map((item, index) => {
      ...
      if (item.itemType === "fixed") {
        const amount = validateMoney(item.amount);
    ...
    if (Object.keys(errors).length) {
      ...
      requestAnimationFrame(() => {
        ...
        document.querySelector<HTMLElement>(`[name="${controlNameFor(firstField)}"]`)?.focus();
      });
      return;
    }
    setLocalFieldErrors({});
    ...
    const normalizedContacts = (contacts: ContactValue[], validated: { value: Array<{ email: string; name: string }> }) => {
      const populated = contacts.filter((contact) => contact.name.trim() || contact.email.trim());
  ...
  return (
    <form id="billing-form" onSubmit={submit}>
        ...
        <div className="panel__body">
          ...
          <div className="form-grid">
            <div className="field field--full">
              ...
              <input aria-describedby={allFieldErrors.name ? "name-error" : undefined} autoFocus className="input" id="name" maxLength={255} name="name" onChange={(event) => setField("name", event.target.value)} placeholder="Ex.: Apartamento 302 — Ed. Aurora" required type="text" value={form.name} />
              <FieldError id="name-error" message={allFieldErrors.name} />
            </div>
            <div className="field field--full">
              <label className="field__label" htmlFor="description">Descrição</label>
              <input aria-describedby={allFieldErrors.description ? "description-error" : undefined} className="input" id="description" maxLength={2000} name="description" onChange={(event) => setField("description", event.target.value)} placeholder="Inquilino, endereço ou nota interna" type="text" value={form.description} />
        ...
        <div className="panel__body">
          ...
          <div className="form-grid">
            <div className="field">
              ...
              <input aria-describedby={allFieldErrors.pix_merchant_name ? "pix-name-error" : undefined} className="input" id="pix_merchant_name" maxLength={25} name="pix_merchant_name" onChange={(event) => setField("pixMerchantName", event.target.value)} placeholder="Até 25 caracteres" type="text" value={form.pixMerchantName} />
            ...
            <div className="field">
              ...
              <input aria-describedby={allFieldErrors.pix_merchant_city ? "pix-city-error" : undefined} className="input mono" id="pix_merchant_city" maxLength={15} name="pix_merchant_city" onChange={(event) => setField("pixMerchantCity", event.target.value)} placeholder="SEM ACENTOS" type="text" value={form.pixMerchantCity} />
      ...
      <div className="actionbar">
        <div className="actionbar__total"><span className="lbl">Subtotal fixo / mês</span><span className="val mono" id="fixed-subtotal">{fixedSubtotal === null ? "Valor inválido" : formatBrl(fixedSubtotal)}</span></div>
