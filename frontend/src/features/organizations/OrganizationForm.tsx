    ...
    const pixStep = (
      <>
        ...
        {customPix ? <div className="organization-pix-fields">
          <div className="field field--full"><label className={labelClass} htmlFor="pix_key">Chave PIX</label><input aria-describedby={describedBy("pix_key", "pix_key-hint")} autoComplete="off" className={`${inputClass} mono`} id="pix_key" name="pix_key" onChange={(event) => update("pix_key", limitApiCharacters(event.target.value, 320))} placeholder={PIX_KEY_GUIDANCE} ref={(element) => { refs.current.pix_key = element; }} spellCheck={false} type="text" value={form.pix_key} /><FieldError id="pix_key-error" message={allFieldErrors.pix_key} /><span className="field__hint" id="pix_key-hint">{PIX_KEY_GUIDANCE}</span></div>
      ...
      <div className="panel">
        ...
        <div className="panel-body">
          ...
          <div className="field">
            ...
            <input aria-describedby={describedBy("pix_key", "edit-pix-key-hint")} autoComplete="off" className={`${inputClass} mono`} id="pix_key" name="pix_key" onChange={(event) => update("pix_key", limitApiCharacters(event.target.value, 320))} placeholder={PIX_KEY_GUIDANCE} ref={(element) => { refs.current.pix_key = element; }} spellCheck={false} type="text" value={form.pix_key} />
