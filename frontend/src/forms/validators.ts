...
export function validateText(value: string, { maxLength, required = false }: { maxLength: number; required?: boolean }): ValidationResult<string> {
  ...
  if (normalized.length > maxLength) return { error: `Informe no máximo ${maxLength} caracteres.` };
...
export function normalizeEmail(value: string): ValidationResult<string> {
  ...
  if (normalized.length > 320) return { error: "Informe no máximo 320 caracteres." };
  return { value: normalized };
}
...
export function validateContacts(contacts: ContactDraft[]): { value: Array<{ email: string; name: string }> } | { errors: Record<string, string> } {
  ...
  contacts.forEach((contact, index) => {
    ...
    else if (name.length > 255) errors[`${index}.name`] = "Informe no máximo 255 caracteres.";
...
export function validatePix(pix: PixDraft): { value: PixDraft } | { errors: Record<string, string> } {
  ...
  if (value.name.length > 25) errors.name = "Informe no máximo 25 caracteres.";
  if (value.city.length > 15) errors.city = "Informe no máximo 15 caracteres.";
...
export function validateUpload(file: File): ValidationResult<File> {
  if (!allowedUploadTypes.has(file.type)) return { error: "Envie um arquivo PDF, JPG ou PNG." };
  if (file.size === 0) return { error: "O arquivo está vazio." };
  if (file.size > maxUploadBytes) return { error: "O arquivo deve ter no máximo 10 MB." };
  return { value: file };
}
